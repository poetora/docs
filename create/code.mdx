...
<Accordion title="自定义语法高亮主题">
  如需自定义主题，请在 `docs.json` 中将主题设为 `"css-variables"`，并使用以 `--poet-` 为前缀的 CSS 变量覆盖语法高亮颜色。

  可用变量如下：

  **基础颜色**

  * `--poet-color-text`: 默认文本颜色
  * `--poet-color-background`: 背景颜色

  **Token 颜色**

  * `--poet-token-constant`: 常量和字面量
  * `--poet-token-string`: 字符串值
  * `--poet-token-comment`: 注释
  * `--poet-token-keyword`: 关键字
  * `--poet-token-parameter`: 函数参数
  * `--poet-token-function`: 函数名
  * `--poet-token-string-expression`: 字符串表达式
  * `--poet-token-punctuation`: 标点符号
  * `--poet-token-link`: 链接

  **ANSI 颜色**

  * `--poet-ansi-black`, `--poet-ansi-black-dim`
  * `--poet-ansi-red`, `--poet-ansi-red-dim`
  * `--poet-ansi-green`, `--poet-ansi-green-dim`
  * `--poet-ansi-yellow`, `--poet-ansi-yellow-dim`
  * `--poet-ansi-blue`, `--poet-ansi-blue-dim`
  * `--poet-ansi-magenta`, `--poet-ansi-magenta-dim`
  * `--poet-ansi-cyan`, `--poet-ansi-cyan-dim`
  * `--poet-ansi-white`, `--poet-ansi-white-dim`
  * `--poet-ansi-bright-black`, `--poet-ansi-bright-black-dim`
  * `--poet-ansi-bright-red`, `--poet-ansi-bright-red-dim`
  * `--poet-ansi-bright-green`, `--poet-ansi-bright-green-dim`
  * `--poet-ansi-bright-yellow`, `--poet-ansi-bright-yellow-dim`
  * `--poet-ansi-bright-blue`, `--poet-ansi-bright-blue-dim`
  * `--poet-ansi-bright-magenta`, `--poet-ansi-bright-magenta-dim`
  * `--poet-ansi-bright-cyan`, `--poet-ansi-bright-cyan-dim`
  * `--poet-ansi-bright-white`, `--poet-ansi-bright-white-dim`
</Accordion>
...
<CodeGroup>
  ...
  const message = adoptPet("Ora", "cat");
  ```
