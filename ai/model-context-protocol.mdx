...
<Warning>
  MCP 服务器仅 Pro 及以上方案可用。
</Warning>

## 关于 MCP 服务器

Model Context Protocol（MCP）是一种开放协议，可在 AI 应用与外部服务（如文档）之间建立标准化连接。Poetora 会基于你的文档和 OpenAPI 规范生成一个 MCP 服务器，使你的内容为更广泛的 AI 生态做好准备，任何 MCP 客户端（如 Claude、Cursor、Goose 等）都可以连接到你的文档和 API。
...
Poetora 会为你的文档自动生成一个 MCP 服务器，并将其托管在你的文档 URL 的 `/mcp` 路径下。例如，Poetora 的 MCP 服务器位于 `https://poetora.com/docs/mcp`。

你可以在[控制台](https://dashboard.poetora.com/products/mcp)查看并复制你的 MCP 服务器 URL。
...
<Tabs>
  <Tab title="Contextual menu">
    在[上下文菜单](/ai/contextual-menu)中为用户添加选项，使其可从文档任意页面连接到你的 MCP 服务器。
  ...
  <Tab title="Claude">
    <Steps>
      <Step title="获取你的 MCP 服务器 URL。">
        前往[控制台](https://dashboard.poetora.com/products/mcp)，找到你的 MCP 服务器 URL。
      </Step>
...
### 示例：连接 Poetora MCP 服务器

连接 Poetora MCP 服务器以与 Poetora API 交互并搜索我们的文档。这样你就能在本地环境中更精准地了解如何使用 Poetora，同时也演示了如何帮助你的用户连接到你的 MCP 服务器。

<Tabs>
  <Tab title="Contextual menu">
    在本页顶部打开上下文菜单，选择 **Connect to Cursor** 或 **Connect to VS Code**，即可将 Poetora MCP 服务器连接到你选择的 IDE。
  </Tab>

  <Tab title="Claude">
    在 Claude 中使用 Poetora MCP 服务器：

    <Steps>
      <Step title="将 Poetora MCP 服务器添加到 Claude">
        1. 进入 Claude 设置中的 [Connectors](https://claude.ai/settings/connectors) 页面。
        2. 选择 **Add custom connector**。
        3. 添加 Poetora MCP 服务器：

        * Name: `Poetora`
        * URL: `https://poetora.com/docs/mcp`
      ...
      <Step title="在聊天中访问 MCP 服务器">
        ...
        2. 选择 Poetora MCP 服务器。
        3. 向 Claude 提问有关 Poetora 的问题。
      </Step>
    </Steps>
  ...
  <Tab title="Claude Code">
    在 Claude Code 中使用 Poetora MCP 服务器，运行以下命令：

    ```bash  theme={null}
    claude mcp add --transport http Poetora https://poetora.com/docs/mcp
    ```
  ...
  <Tab title="Cursor">
    <PreviewButton href="cursor://anysphere.cursor-deeplink/mcp/install?name=poetora&config=eyJ1cmwiOiJodHRwczovL3BvZXRvcmEuY29tL2RvY3MvbWNwIn0=">在 Cursor 中安装</PreviewButton>

    要将 Poetora MCP 服务器连接到 Cursor，点击 **在 Cursor 中安装** 按钮。若需手动连接 MCP 服务器，请按以下步骤操作：
    ...
    <Steps>
      ...
      <Step title="测试连接">
        在 Cursor 的聊天中，输入 “What tools do you have available?”。Cursor 应显示 Poetora MCP 服务器为可用工具。
      </Step>
    </Steps>
  ...
  <Tab title="VS Code">
    <PreviewButton href="https://vscode.dev/redirect/mcp/install?name=poetora&config=%7B%22type%22%3A%22http%22%2C%22url%22%3A%22https%3A%2F%2Fpoetora.com%2Fdocs%2Fmcp%22%7D">在 VS Code 中安装</PreviewButton>

    要将 Poetora MCP 服务器连接到 VS Code，点击 **在 VS Code 中安装** 按钮。若需手动连接 MCP 服务器，创建 `.vscode/mcp.json` 文件并添加：
...
<AccordionGroup>
  <Accordion title="MCP 服务器仅显示搜索工具">
    ...
    2. 确认已通过 `x-poet.mcp.enabled: true` 明确为相关端点启用 MCP。
